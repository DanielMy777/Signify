...
import { Camera, useCameraDevices, useFrameProcessor, Frame, sortFormats } from 'react-native-vision-camera';
...
const App = () => {
  ...
  const wtf = useSharedValue({ ok: true, m: () => { console.log('sup') } });
  useEffect(() => {

    const check_premessions = async () => {


      Orientation.getOrientation((value) => { orientation_obj.value = value })
  ...
  const upload_img = async (img) => {
    try {
      ...
      res = await http_method('http://192.168.1.125:2718/api/img', HttpMethod.POST, { img: img });
      console.log(res);
    }
    catch (err) {
  ...
  return (
    <View style={styles.container}>
      <ReanimatedCamera
        ...
        frameProcessorFps={1}
      />
      <Button title='flip camera' styles={styles.button} onPress={() => { setFrontCamera(!frontCamera); isFrontCamera.value = !isFrontCamera.value }} />
      <Text style={styles.text}> {frontCamera ? 'detected Or' : 'detected Screen'}</Text>
    </View>
